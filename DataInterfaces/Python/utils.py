...
def ssdb_habits(habit_id, printinfo=False):
  ...
  #2021-02-19 Vasileios Barlakas: Adapted to include multiple occurrences of the same habit (TRO and ARO).
  ...
  -------
  folder: str
    Full path to folders for specified habit.
  """
  ...
  if isinstance(habit_id,str):
    ...
    if len(hs) < n:
      for i in np.arange(len(hs)):
        if counts[i] > 1:
         print( 'Habit id %d is found %d times.\n' %(hs[i], counts[i]) )
      #assert(len(hs)==n), \
  ...
  #- Extract folder for specified habit
    ...
    if habit_id==-1:
      ...
      # .index returns the folder where the habit_id is found first
      #folder = ssdb_habits.folders[ssdb_habits.habits.index(habit_id)]

      # Returns all folder occurrences for the same habit_id
      folder_index = [index for index, value in enumerate(ssdb_habits.habits) if value == habit_id]
      folder = [ssdb_habits.folders[i] for i in folder_index]
...
def ssdb_display(habit_id=None, orientation=None, dveq=None):
  ...
  #2021-02-19 Vasileios Barlakas: Fixed bug when both habit_id and orientation are None; reconstructed according to Matlab interface.
  #                               Fixed bug when a habit at hand exists in multiple orientations.
  ...
  orientation: str
    descriptor of orientation to explore for the chosen habit, e.g., 'totally_random', 'azimuthally_random_beta000.0deg'
  dveq: float (or int)
  ...
  if habit_id is None:
    ...
    # Main folders describing the phase of the hydrometeor
    ...
    riming            = []
    habit_description = []
    for ihabit in habits:
        habit_folder = ssdb_habits( ihabit )
        partialpaths = habit_folder[0].split(os.sep)

        phase.append(            partialpaths[-4] )
        aggregation.append(      partialpaths[-3] )
        riming.append(           partialpaths[-2] )
        habit_description.append(partialpaths[-1] )
    ...
    for i1 in np.arange(len(main_folders)):
        hits1 = [index for index, value in enumerate(phase) if value == main_folders[i1]]
        # Only if a habit exists at the given phase
        if len(hits1)>0:
            hits1  = np.array(hits1)
            aggr_  = np.array(aggregation)
            names2 = np.unique(aggr_[hits1])
            if names2.shape[0]>0:
                print( '%s\n---' %main_folders[i1] )
            for i2 in np.arange(names2.shape[0]):
                hits2  = [index for index, value in enumerate(aggregation) if value == names2[i2]]
                hits2  = np.array(hits2)
                rim_   = np.array(riming)
                names3 = np.unique(rim_[hits2])
                if names3.shape[0]>0:
                    print( ' %s' %names2[i2] )
                for i3 in np.arange(names3.shape[0]):
                    hits3  = [index for index, value in enumerate(riming) if value == names3[i3]]
                    hd_    = np.array(habit_description)
                    hits4  = [value for value in hits1 if value in hits2 and value in hits3]
                    hits4  = np.array(hits4)
                    names4 = np.unique(hd_[hits4])
                    if names4.shape[0]>0:
                        print( '  %s' %names3[i3] )
                    for i4 in np.arange(names4.shape[0]):
                        ...
                        print( '   %3s %3d: %s%sa=%.1e / b=%.2f' \
                          %( status_symbol, habit_id, S['HABIT_NAME'],
                             (43-len(S['HABIT_NAME']))*bs,
                             float(S['ALPHA']), float(S['BETA']) ) )
                        habit_folders = ssdb_habits(habit_id)
                        orient = []
                        for ifolder in habit_folders:
                            content = os.listdir(ifolder)
                            if 'TotallyRandom' in content:
                                orient.append('totally_random')
                            if 'TotallyRandom' not in content:
                                orient.append('azimuthally_random')
                        for i in orient:
                            print('          %s' %i)

        else:   # if a phase (e.g., "Mixed") does not exist yet
            pass
    print( '' )
    print( '-------------------------------------------------------------------------')
    print( '' )
  # habit & orientation specified: derive and print info sizes available for this habit-orientation-combi
    ...
    # Get all habit folders (i.e., orientations) containing the same habit id
    habit_folders = ssdb_habits(habit_id)

    # Loop over different folders containing the same habit id
    for ifolder in habit_folders:
        content = os.listdir(ifolder)
        if 'TotallyRandom' in content:
            if orientation == 'totally_random':
                habit_folder = ifolder
        else:
            if orientation != 'totally_random':
                habit_folder = ifolder
    ...
    if orientation != 'totally_random':
        ibeta        = [i for i,x in enumerate(orientations) if orientation in x][0]
        ofolders     = [ofolders[ibeta]]
        orientations = [orientations[ibeta]]
        tilt_angles  = [tilt_angles[ibeta]]

    # Get the basic information by combining existing functions
    S = ssdb_summary_folder( habit_folder, printinfo=False )
...
def ssdb_summary( habit_id, printinfo=True ):
  ...
  for ihabit in habit_folder:

      summary_file = ssdb_summary_find( ihabit )
      S = ssdb_summary_read( summary_file )
...
def ssdb_summary_folder( hfolder, printinfo=True ):
  #2021-02-19 Vasileios Barlakas: As in ssdb_summary, but for a given habit folder
  """Obtains the habit summary data.

  The functions obtains the summary data for given habit folder.
  ...
  S: dict
    Dictionary with summary data.
  """
  summary_file = ssdb_summary_find( hfolder )
  S = ssdb_summary_read( summary_file )
...
def ssdb_import_habit(habit_id, orientation,
  ...
  #2021-02-19 Vasileios Barlakas: Extended to handle habits found within DB in multiple orientations.
  ...
  *allow_nodata* is set. For example, for one size there can be a single
  ...
  if (habit_folder is None):
    habit_folders = ssdb_habits(habitID)
